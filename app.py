...
# モンスター名リストの定義（monster_list.pyがない場合用）
MONSTER_NAMES = [
...
def handle_message(event):
    ...
    # 属性だけ入力された場合 (例: 火属性) → スキル検索を優先
    # 以下の行をコメントアウトまたは削除
    # for element in ELEMENTS:
    #     element_pattern = f"{element}属性"
    #     if text == element_pattern:
    #         result = search_by_weakness(element_pattern)
    #         line_bot_api.reply_message(reply_token, TextSendMessage(text=result))
    #         return
    ...
    # 弱点属性検索: 「弱点 火」「弱点 水属性」のようなパターン
    weakness_pattern = r'^(?:弱点\s+([火水雷氷龍])(?:属性)?|([火水雷氷龍])属性(?:\s+弱点|\s*弱い|弱点))
