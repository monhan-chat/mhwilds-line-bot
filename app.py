# 装備から検索用辞書作成
    for skill in skills_data:
        for armor in skill.get("装備", []):
            ...
            # 防具名から基本名を抽出（αやβを除去）
            base_name = armor_name.rstrip('αβ')
            
            if base_name:
                # 基本名と完全一致する装備も、部分一致する装備も登録
                armor_to_skill[base_name] = skill["スキル名"]
...
def handle_message(event):
    ...
    if not result:
        # 最初に完全一致を試み、なければ部分一致
        full_match = [armor for armor in armor_to_skill.keys() if text == armor]
        partial_match = [armor for armor in armor_to_skill.keys() if text in armor]
        
        matched_armors = full_match or partial_match
        
        if matched_armors:
            armor_name = matched_armors[0]  # 最初の一致した防具を使用
    ...
    if result:
        ...
        if result.get("装備"):
            ...
            # JSONの元の順序を保持しつつ、スロット情報を表示
            for armor in result["装備"]:
                # スロット情報の可視化
                slots = armor.get('スロット', [])
                slot_count = sum(1 for slot in slots if isinstance(slot, int))
                slot_str = '□' * slot_count
    ...
    if result:
        ...
        if result.get("装備"):
            ...
            for armor in result["装備"]:
                # スロット情報の可視化
                slots = armor.get('スロット', [])
                # スロットの値が数値の配列の場合、その配列の長さをスロット数とする
                slot_count = sum(1 for slot in slots if isinstance(slot, int))
                slot_str = '□' * slot_count
